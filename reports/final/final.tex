...
The naive implementation was run on a small graph designed by hand and it gave
meaningful results (vertices connected to the graph in a similar way have a small
$\|\mathbf{C_i} - \mathbf{C_j}\|^2$) but it does not scale for large datasets.

For large graphs, it is expected to have sparse adjacency matrices, where the
