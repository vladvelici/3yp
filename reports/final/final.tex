...
Same as for the undirected graphs, an approximation of the cloud vectors can be
made using only the $k$ largest eigenvalues and eigenvectors. Let
$\bm{\hat{\Lambda}}$ be a diagonal matrix containing the leading $k$ eigenvalues,
$\bm{\hat{\vee}_R}$ be a matrix that contains the leading $k$ right eigenvectors,
and $\bm{\hat{\vee}_L}$ a matrix of the leading $k$ left eigenvectors. Arnoldi
Iteration is an efficient iterative algorithm to compute the leading eigenvalues
and eigenvectors of large sparse matrices \cite{lehoucq1996deflation}. Some of
the computed eigenvalues might be complex, so is it necessary to check for all
complex eigenvalues if their complex conjugates are also computed, and drop the
eigenvalues (and corresponding left and right eigenvectors) if it is not. As a
result of this extra check, the actual number of eigenvalues and eigenvectors
used might be less than the initial $k$, however in the rest of this subsection,
the variable $k$ will be used to denote this number.
...
take $O(k^3N)$ space. The time complexity of computing the approximative dot
