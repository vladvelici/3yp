...
edges removed, and {\tt 05removed\_fb.csv} is a list of the edges removed.
The {\tt conncomp} tool command used is:
\begin{lstlisting}
...
\begin{equation}
  \bpos(a, \ttop(i)) = \pos(a, \ttop(i)) - \sum_{(i, k) \in \text{Blacklist}}
  \Big[ 1 \text{  if  } \pos(k, \ttop(i)) < \pos(a, \ttop(i)) \Big].
\end{equation}
...
more information. A plot of this averate versus the number of eigenvalues used
is in \emph{Figure~\ref{fig:fb_eig_pos}}, from which can be conlcuded that the
algorithm performs best at $k=40$ eigenvalues, because at lower and higher
values of $k$, the average is much higher. Despite the fact that the
percentage of removed edges in top 5 increases with $k$, the average does not
decrease with the increase of $k$, fact explained by the similarities outside of
top 5 being very low (thus being placed far in the top).
...
that they are not in the list of removed edges or existing edges and it is never
a pair like $(i, i)$. The average percentage (from three runs) of the edges that
have smaller scores (higher similarities) than random vs the number of
eigenvalues used, $k$, is plotted in \emph{Figure~\ref{fig:fb_eig_better}}.
It can be observed that the it peaks $k=40$ and $k=50$ with values between
$88.1\%$ and $88.2\%$, decreasing for higher or smaller values of $k$.
...
\begin{figure}[tpb]
  ...
  \begin{subfigure}[b]{0.5\textwidth}
    \includegraphics[width=\textwidth]{fb_eig_scores}
		\caption{Removed edges and relative scores.}
  ...
  (score). The penalising factor used is $\mu = 0.5$.}
