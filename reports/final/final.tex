...
Same as for undirected graphs, an approximation of the cloud vectors can be
made using only a small number of the largest eigenvalues and eigenvectors. Some
of the eigenvalues and corresponding (left or right) eigenvectors might be
complex. The cloud vectors only contain real values, thus it is necessary to
make sure their approximation is also real. The approximation is real if, for
every complex eigenvalue used, its complex conjugate is also used. The
eigenvectors are complex only if their corresponding eigenvalues are complex, and
if two eigenvalues are complex conjugates, then their corresponding eigenvectors
are also complex conjugates. It is, therefore, sufficient to check that for each
complex eigenvalue used, its conjugate is also used.
