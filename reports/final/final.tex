...
%% TODO: why cholesky does not work.
...
example, on a friends dataset of 2212 vertices and 37397 edges, the maximum depth
heuristic with depth 5 has reduced the number of comparisons by 47.93\%, and
only reduced the prediction accuracy by x.

%% TODO: heuristic performance analysis
...
presented in \emph{Table~\ref{tbl:interface}}. Some helper functions are not
...
\begin{table}[!ht]\footnotesize
   ...
   \end{tabular}
   \caption{Implementation interface. Parameters: q represents the matrix
    $\mat{Q}$, z represents the matrix $\mat{Z}$, adj represents the adjacency
    matrix, and i and j represent vertex numbers.}
   \label{tbl:interface}
\end{table}
...
index. It has the following methods:

\begin{lstlisting}
  _dotprod(a, b)   Computes the dot product between vertices a and b.
  score(a, b)      Computes the Euclidean distance score.
  save(f)          Writes the index into file f.
  nodelist()       Returns a list of all vertices.
  nid(a)           Returns the index of vertex a in the index.
  __len__()        Returns the number of vertices.
\end{lstlisting}

Internally, a {\tt Sim} object has either a $\bm{\omega}$ matrix or a pair of
$\mat{Q}$ and $\mat{Z}$, which it uses to compute the dot products and the
similarities. The implementation of {\tt Sim.nib(a)} is simply returning {\tt a}.
This is because the {\tt Sim} class represents an index directly trained from
an adjacency matrix. In practice, vertices usually come with different IDs,
strings or integers that do not start at 1 or 0.

The class {\tt Simp} extends the class {\tt Sim}. To promote an object of type
{\tt Sim} to {\tt Simp}, it needs a {\tt Provider}. Any object that can create
...
an object is required to implement to be a {\tt Provider} is shown below:

\begin{lstlisting}
  __getitem__(a)    Get the index of a in the adj. matrix.
  __len__()         Return the number of vertices.
  adj()             Get the adjacency matrix.
  nodelist()        Return a list of all vertices.
  save(f)           Saves itself in the file f.
\end{lstlisting}
...
In the package {\tt sim} there are some important functions defined:

\begin{lstlisting}
  train(adj, mu, k)             Calls train_directed or train_undirected,
                                depending on whether adj is symmetric.
  train_undirected(adj, mu, k)  Undirected algorithm to obtain omega.
  train_directed(ajd, mu, k)    Directed algorithm to obtain Q and Z.

  All train*() functions return a Sim object.

  prov(provider, sim)           Combines the given sim with the given
                                provider and returns a Simp object.
  loadp(path)                   Load an index (Sim or Simp) from path.
\end{lstlisting}
...
of the friends relationships from the social network Facebook. The data was
given away by volutneers for a Kaggle competition and it has been fully
anonimised by Kaggle before making it available to the public. % TODO: citation

The friends dataset was converted to a CSV edge list graph (using a small Go
program) where a vertex represents a person and an edge represents a
bidirectional friendship. The graph has $26,457$ vertices and $372,227$
undirected edges, but it is divided into $378$ connected components. The largest
connected component (by the number of vertices) has $2212$ vertices and $41552$
undirected edges. The top 10 largest components is shown in
\emph{Table~\ref{tbl:conncomp}}.

\begin{table}[!ht]\footnotesize
  \centering
  \begin{tabular}{p{3cm}p{2cm}l}
    \textbf{Component} & \textbf{Vertices} & \textbf{Edges} \\ \hline
    {\tt 4\_all.csv  }  & 2212     & 41552 \\
    {\tt 10\_all.csv }  & 1700     & 36179 \\
    {\tt 3\_all.csv  }  & 1304     & 23029 \\
    {\tt 5\_all.csv  }  & 1216     & 13772 \\
    {\tt 9\_all.csv  }  & 942      & 6743  \\
    {\tt 23\_all.csv }  & 724      & 14712 \\
    {\tt 8\_all.csv  }  & 653      & 21597 \\
    {\tt 27\_all.csv }  & 630      & 7865  \\
    {\tt 14\_all.csv }  & 617      & 9446  \\
    {\tt 39\_all.csv }  & 527      & 24838
  \end{tabular}
  \caption{Top 10 connected components by the number of vertices in the friends
   dataset.}
  \label{tbl:conncomp}
\end{table}
