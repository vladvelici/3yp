...
The matrices $W^{\sfrac{1}{2}}$ and $W^{\sfrac{-1}{2}}$ are easy to compute
because $W$ is a diagonal matrix. Apply the operation only on the diagonal
elements and obtain $(W^{\sfrac{1}{2}})_{ii} = (W_{ii})^{\sfrac{1}{2}}$ and
$(W^{\sfrac{-1}{2}})_{ii} = (W_{ii})^{\sfrac{-1}{2}}$.
...
Let $\lambda_i$ be the $i^{th}$ eigenvalue of $A$ and $\bm{\vee^{(i)}}$ be the
$i^{th}$ eigenvector of $A$ ($\forall i \in \{ 1, 2, ..., N \}$). $\vee$ is a
matrix of all $N$ eigenvectors (such that $\bm{\vee^{(i)}}$ is the $i^{th}$
column of $\vee$), and $\Lambda$ is a diagonal matrix of all $N$ eigenvalues
%
\begin{align}
  \vee &= \begin{bmatrix}
    \vert           & \vert           &       & \vert \\
...
The diagonalisation of the matrix A is $A = \vee \Lambda \vee^{-1}$. A is a real
symmetric matrix thus its eigenvectors are orthogonal, which makes $\vee$ an
orthogonal matrix, therefore $\vee^{-1} = \vee^{T}$ \cite{eigval_and_eigvec}.
The diagonalisation of $A$ is
%
\begin{align}
  A &= \vee \Lambda \vee^{T} \\
  \Rightarrow A^t &= \big(\vee \Lambda \vee^{T}\big)^t
                  = \vee \Lambda^t \vee^{T} \\
  \label{eq:a-at-t-sum}
...
\begin{equation}
  ...
  D^t = W^{\sfrac{1}{2}} \sum_{a=1}^{N} \lambda_a^t \bm{\vee^{(a)}} \bm{\vee^{(a)^T}}
    W^{\sfrac{-1}{2}}.
\end{equation}
...
\begin{equation}
  \bm{c}_i = \sum_{t=0}^\infty \mu^t W^{\sfrac{1}{2}} \sum_{a=1}^{N} \lambda_a^t
    \bm{\vee^{(a)}} \bm{\vee^{(a)^T}} W^{\sfrac{-1}{2}} \bm{\delta}_i,
\end{equation}
rearrange the equation and obtain
\begin{equation}
  \label{eq:ci-rearranged}
  \bm{c}_i = W^{\sfrac{1}{2}} \sum_{a=1}^{N} \sum_{t=0}^\infty \mu^t \lambda_a^t
    \bm{\vee^{(a)}} \bm{\vee^{(a)^T}} W^{\sfrac{-1}{2}} \bm{\delta}_i.
\end{equation}
...
Observe $\bm{\vee^{(a)^T}} W^{\sfrac{-1}{2}} \bm{\delta}_i$ is a scalar
\begin{equation}
  \label{eq:observe-scalar}
  \bm{\vee^{(a)^T}} W^{\sfrac{-1}{2}} \bm{\delta}_i = \bm{\vee^{(a)}}_i.
    W^{\sfrac{-1}{2}}_{ii}
\end{equation}
...
\begin{equation}
  ...
  \bm{c}_i = W^{\sfrac{1}{2}} \sum_{a=1}^{N} \frac{1}{1 - \mu \lambda_a}
    \bm{\vee^{(a)}} \bm{\vee^{(a)}}_i W^{\sfrac{-1}{2}}_{ii}.
\end{equation}
...
An approximation of the vectors $\bm{c}_i (\forall i \in v)$ can be obtained
by only using the $k$ leading eigenvectors and eigenvalues of $A$ instead of all
of them. It is expected that the $A$ is large and sparse, because in a typical
dataset the number of relationships is significantly smaller than the number
of data points squared. In terms of the undirected graph $\mathcal{G}(\mathcal{V},
\mathcal{E})$ it is expected that $\|E\| \ll \|V\|^2$. The Lanczos Method is an
efficient iterative algorithm to compute the leading eigenvalues of large sparse
real symmetric matrices \cite{calvetti1994implicitly} and it is implemented in
both Matlab and Python (SciPy).

Compute the largest $k$ eigenvalues and eigenvectors using the Lanczos Method
and define $\hat{\vee}$ to be the matrix of the $k$ leading eigenvectors and
$\hat{\Lambda}$ to be the diagonal matrix of the leading $k$ leading eigenvalues
\begin{align}
  \hat{\vee} &= \begin{bmatrix}
    \vert           & \vert           &       & \vert \\
...
\begin{equation}
  \label{eq:c-hat}
  \bm{\hat{c}}_i = W^{\sfrac{1}{2}} \sum_{a=1}^{k} \frac{1}{1 - \mu
    \lambda_a} \bm{\vee^{(a)}} \bm{\vee^{(a)}}_i W^{\sfrac{-1}{2}}_{ii}.
\end{equation}
...
To simplify the equation, let $Z$ be a $N \times k$ matrix such that
\begin{align}
  \label{eq:z-ij}
  Z_{ij} &= \frac{\bm{\vee^{(j)}}_i W^{\sfrac{-1}{2}}_{ii}}{1-\mu \lambda_j},
\end{align}
...
The computation of $Z$ (\emph{Equation~\ref{eq:z-ij}}) can be vectorised using a
new $k \times k$ diagonal matrix $R$ as follows
\begin{align}
  \label{eq:z-vectorised}
  Z = W^{\sfrac{-1}{2}} \hat{\vee} R, &&
  R_{ii} = \frac{1}{1 - \mu \lambda_i}.
\end{align}
