...
%
The eigendecomposition of the matrix A is $A = \vee \Lambda \vee^{-1}$. The graph $G$ is an undirected graph which makes $A$ a real symmetric matrix, thus $\vee^{-1} = \vee^{T}$. The eigendecomposition of the matrix $A$ is therefore:
\begin{align}
A &= \vee \Lambda \vee^{T} \\
