...
\par
To objectively evaluate how well the outputs of the algorithm reflect the reality of the dataset it is being used on, we need to carefully define the goals of the algorithm in respect of the dataset. We only look for datasets where the \textit{similarities} between entities can be obtained exclusively from the graph structure. We need labelled (known \textit{similarities} between entities) datasets that have only one relationship type from which we can exclusively compute \textit{similarities} between nodes.
\par
