...
\par
A random connected graph of $N=200$ vertices and $350$ edges was generated. The similarities between all pairs of nodes were computed using both the brute-force approach and the approximative approach, and nodes were sorted by their similarities to obtain $200$ tops (the higher the position in the top, the more similar the nodes are). Let $B^m_{ij}$ be the position of node $j$ in the top similar nodes of node $i$ for the brute-force approach with $m$ iterations. Similarly let $T^{m}_{ij}$ be the position of node $j$ in the top similar nodes of node $i$ for the approximative approach with $m$ eigenvectors. Define the difference between two approaches to be: 
\begin{equation} d(B^m, T^{m'}) = \frac{1}{N^3} \sum_{i=1}^N \sum_{j=1}^N \operatorname{abs}(B^m_{ij} - T^{m'}_{ij}) \end{equation}
%
The results can be observed in \emph{Figure~\ref{fig:no-of-eigenvectors}}. The choice of the penalising factor $\mu=0.5$ was used for all the runs. The difference d was evaluated between the brute-force approach with $m=20$ iterations and the approximative approach with $m' = 5 to 195$ eigenvectors. We can observe that if we use the majority of eigenvalues, the difference is insignificant. It is computationally unfeasible to do that on a large dataset thus we will aim for a number of eigenvectors $m'$ in the range of $20$ to $40$, where the difference seems to be at a local minimum. The local minimum might differ from dataset to dataset depending mainly on the size of the dataset. For completeness, the experiment was run on a randomly generated graph with $500$ vertices and $700$ edges. The first local minimum of d() was at around 100 eigenvectors with a difference of approx. $0.2$, and at $40$ eigenvectors the difference is $0.2297$, and at $20$ eigenvectors the difference is $0.2557$.
\begin{figure}[tpbh]
\centering
