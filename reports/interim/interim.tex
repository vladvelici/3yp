...
\end{figure}
%
The results can be observed in \emph{Figure~\ref{fig:no-of-eigenvectors}}. The choice of the penalising factor $\mu=0.5$ was used for all the runs. The difference d was evaluated between the brute-force approach with $m=20$ iterations and the approximative approach with $m' = 5 to 195$ eigenvectors. We can observe that if we use the majority of eigenvalues, the value of $d()$ is very small. It is computationally unfeasible to do that on a large dataset thus we will aim for a number of eigenvectors $m'$ in the range of $20$ to $40$, where the difference seems to be at a local minimum (on this small dataset). The local minimum might differ from dataset to dataset depending mainly on the size of the dataset. For completeness, the experiment was run on a randomly generated connected graph with $500$ vertices and $700$ edges. The first local minimum of $d()$ was at around 100 eigenvectors with a value of approx. $0.2$. At $40$ and $20$ eigenvectors, the values of $d()$ obtained are $0.2297$ and $0.2557$, respectively.
%
%
% -----
...
%
A subjective method of evaluation on real data is attempting to visualise the algorithm output on real data and deduce whether it gives sensitive results. This method has the disadvantage of being subjective but is nonetheless a start of the journey of building an evaluation method for the algorithm.
%
...
\par
To objectively evaluate how well the outputs of the algorithm reflect the reality of the dataset it is being used on, we need to carefully define the goals of the algorithm in regards the the dataset. We only look for datasets where the \textit{similarities} between entities can be obtained exclusively from the graph structure. We need labelled (known \textit{similarities} between entities) datasets that have only one relationship type from which we can exclusively compute \textit{similarities} between nodes.
\par
If the dataset used for evaluation has more than one type of relationship (more features), the evaluation is subjective to the choice of the weights for each relationship type. The reference similarities (from dataset or obtained by other means) might be based on different weights of each feature of the dataset, therefore using equal weights in the graph representation will likely lead to a poorer performance and biased evaluation. 
\par
Given some datasets that fit the requirements from above, we can run our algorithm and compare the results with the reference similarities. In reality datasets have many features and a complex graph structure with relationships having different weights, therefore this evaluation method cannot be done in practice.
\par
A similar way is to compute similarities using another algorithm (call it reference algorithm) and only one feature of a dataset. Use the same feature to build a graph structure, then compare the results of our algorithm with the results of the reference algorithm used.
\par
Given a labelled dataset that fits the requirements from above, we can run our algorithm and compare the results with the reference similarities. A single dataset can result in biased evaluations therefore more such datasets are required. The downside is that datasets with known similarities between entities are not very common, and because the term \textit{similarity} is ambiguous (e.g. why is an entity \textit{similar} to another? In regards to which features?). Given an unlabelled dataset, we can obtain a labelled dataset using other algorithms and techniques, but the comparison will be in regards to the labelling method used.
\par
An online evaluation is a way to evaluate the system by getting feedback from users. In this case, we would deploy the algorithm on a experimentation website and display the results to live users, while making sure to track the way users interact with the outputs of the algorithm.
%-------------------------------------------------------------------------------
% LIMITATIONS
%-------------------------------------------------------------------------------
