...
\par
It is not computationally feasible to calculate the exact similarities between vertices of very large graphs. This project investigates a method to compute an approximation of similarities between nodes and attempt to evaluate its performance (accuracy) on different datasets.
\par
...
\par
For the implementation of both the approximative method and the brute-force algorithm, sparse matrices were used. A sparse matrix in Matlab only stores the non-zero values of a matrix, therefore a sparse matrix only uses memory for the positions where it holds values. For instance, to store the adjacency matrix of a very large graph $G(v,\epsilon)$, using a dense matrix would take $O(|v|^2)$ memory whereas using a sparse matrix only takes $O(|\epsilon|)$ memory.
\par
The approximative method was implemented in Matlab as a couple of Matlab functions. The function signatures along with descriptions is presented in \emph{Table~\ref{tbl:interface}}. q represents the matrix $Q$, z represents the matrix $Z$, adj represents the adjacency matrix, and i and j represent vertex numbers.
...
A simple evaluation metric of how well the approximative method retains the most important information from the graph is to evaluate how different are the top similar vertices for the brute-force approach and the approximative method with different number of eigenvalues and eigenvectors used.
\par
A random connected graph of $N=200$ vertices and $350$ edges was generated. The similarities between all pairs of nodes were computed using both the brute-force approach and the approximative method, and nodes were sorted by their similarities to obtain $200$ tops (the higher the position in the top, the more similar the nodes are). Let $B_{ij}$ be the position of node $j$ in the top similar nodes of node $i$ for the brute-force approach. Similarly let $T^{20}_{ij}$ be the position of node $j$ in the top similar nodes of node $i$ for the approximative approach with 20 eigenvectors. Define the difference between the two methods to be 
\begin{equation} d = \sum_{i=1}^N \sum_{j=1}^N \operatorname{abs}(B_{ij} - T_{ij}) \end{equation}
%
%-------------------------------------------------------------------------------
% LIMITATIONS
%-------------------------------------------------------------------------------
